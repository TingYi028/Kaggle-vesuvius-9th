...
from ..optimizer.nnUNetTrainerMuSGD import MuSGD
...
class nnUNetTrainerMedialSurfaceRecall_MuSGD(nnUNetTrainerSkeletonRecall):
    @staticmethod
    def get_training_transforms(
            patch_size: Union[np.ndarray, Tuple[int]],
            rotation_for_DA: RandomScalar,
...
class nnUNetTrainerMedialSurfaceRecall_MuSGD_100E(nnUNetTrainerSkeletonRecall):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), yaml_config_path: Optional[str] = None):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device, yaml_config_path)
        self.weight_srec = 1  # This is the default value, you can change it if you want
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        return optimizer, lr_scheduler
class nnUNetTrainerMedialSurfaceRecall_MuSGD_250E(nnUNetTrainerSkeletonRecall):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), yaml_config_path: Optional[str] = None):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device, yaml_config_path)
        self.weight_srec = 1  # This is the default value, you can change it if you want
