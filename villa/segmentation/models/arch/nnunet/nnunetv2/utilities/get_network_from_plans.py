...
def get_network_from_plans(arch_class_name, arch_kwargs, arch_kwargs_req_import, input_channels, output_channels,
    ...
    for ri in arch_kwargs_req_import:
        ...
        # 新增以下這段
    import torch
    if architecture_kwargs.get('norm_op') is torch.nn.GroupNorm:
        norm_kwargs = architecture_kwargs.get('norm_op_kwargs') or {}
        ng = norm_kwargs.pop('num_groups', 32)

        def gn(c, ng=ng, norm_kwargs=norm_kwargs):
            return torch.nn.GroupNorm(num_groups=ng, num_channels=c, **norm_kwargs)

        architecture_kwargs['norm_op'] = gn
        architecture_kwargs['norm_op_kwargs'] = {}  # 很重要：避免 eps/affine 再被傳一次
